...
# batch_size_val = 1
# val_num = 0
tra_img_name_list = glob.glob(f"{data_dir}/{tra_image_dir}/TRA_RO_*{image_ext}")
...
tra_lbl_name_list = glob.glob(f"{data_dir}/{tra_label_dir}/TRA_RO_*{label_ext}")
...
tra_bbox_name_list = glob.glob(f"{data_dir}/{tra_bbox_dir}/TRA_RO_*{bbox_ext}")
...
# tra_img_name_list = tra_img_name_list[:1000]
# tra_lbl_name_list = tra_lbl_name_list[:1000]
print("---")
...
if torch.cuda.is_available():
    net.cuda()
pretrained_model_path = 'saved_model/train_4road_last.pth'
...
for epoch in range(0, epoch_num):
    ...
    scheduler.step()
# tra_lbl_name_list = []
torch.save(net.state_dict(), model_dir + "train_4road_new.pth")
plt.plot(loss_list)
plt.title("train_4road_loss_curve")
plt.savefig('train_4road_loss_curve.png')
